...
LOG_MODULE_REGISTER(simple_tx,LOG_LEVEL_DBG);
...
static void tx_thread(void *p1, void *p2, void *p3)
{
	...
	while (true) {
		...
		/* Transmit data through radio */
		int ret = radio_api->tx(ieee802154_dev, IEEE802154_TX_MODE_DIRECT,
				pkt, buf);
		if (ret) {
			LOG_ERR("Error transmit data");
		}
...
int main(void)
{
	...
	LOG_INF("Starting simple_tx application");
